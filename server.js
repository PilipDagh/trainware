// server.js (Part 1 - Setup, Game State, and Player Movement)
...
const ROLES =[
    ...
    { name: 'Normal', desc: 'Just a regular passenger' }
];
...
class Lobby {
    constructor(id) {
        ...
        this.train = {
            ...
            targetSpeed: 0,
            maxSpeed: 35,
            accel: 35 / 8, // Takes 8 seconds to reach 35 km/h
            fuel: 1003,
            maxFuel: 1003,
            fuelBurn: 17, // 17 fuel per second
            pressure: 0,
            lastPressureDecay: 0,
            isMoving: false,
            departureTimer: 0,
            cooldownTimer: 0,
            stopEventTriggered: false,
            speedUpgraded: false,
        ...
        this.world = {
            ...
            steamClouds: [], bullets: [], marshals:[]
        };
        this.state = {
            biome: 'Forest', tunnelActive: false, lastTunnelCheck: 0,
            bridgeOut: false, bridgeDistance: 500 + Math.random() * 1000,
            victory: false, townCounter: 0, nextTownDist: Math.floor(Math.random() * (497 - 274 + 1)) + 274
        };
        this.stats = { 
            leadSlinger: { name: 'None', dmg: 0 }, 
            drunkard: { name: 'None', sips: 0 }, 
            workhorse: { name: 'None', mined: 0 }, 
            snake: { name: 'None', betrayals: 0 } 
        };
        this.lastTick = Date.now();
        this.votesToRestart = new Set();
    }

    getBiome() {
        let d = this.train.distance;
        if (d < 974) return 'Forest';
        if (d < 1521) return 'Desert'; // 974 + 547
        if (d < 1908) return 'Tundra'; // 1521 + 387
        if (d < 2455) return 'Desert'; // 1908 + 547
        return 'Forest'; // Final 726 km
    }
}
...
io.on('connection', (socket) => {
    ...
    function joinLobby(socket, lobbyId, name, isTraitor) {
        ...
        lobby.players[socket.id] = {
            ...
            x: 500, y: 300, // Start in passenger car
            hp: 100, maxHp: 100,
            role: role, 
            isTraitor: isTraitor,
            inventory: { 
                gold: 0, silver: 0, coal: 0, rawMeat: 0, cookedMeat: 0, 
                skins: role.name === 'Trapper' ? 3 : 0, 
                watches: 0, 
                ammo: role.name === 'Soldier' ? 62 : 30, 
                money: 0,
                medkits: 0,
                beer: 0
            },
            ...
            spectateIndex: 0,
            sips: 0, drunkTimer: 0, 
            flashlight: false, lightOn: false,
    ...
    socket.on('input', (data) => {
        ...
        // Drunk speed boost (3 to 10 sips)
        if (p.sips >= 3 && p.sips < 11) speed *= 1.5; 
        ...
        // Train boundaries
        let trainStartX = 0;
        let trainEndX = lobby.train.cars[lobby.train.cars.length - 1].x + lobby.train.cars[lobby.train.cars.length - 1].w;
        let trainTopY = 250;
        let trainBottomY = 350;

        // Physical barrier if train is moving
        if (lobby.train.speed > 0) {
            // If player is currently on the train
            if (p.y >= trainTopY && p.y <= trainBottomY && p.x >= trainStartX && p.x <= trainEndX) {
                // Prevent walking off
                p.x = Math.max(trainStartX, Math.min(trainEndX, newX));
                p.y = Math.max(trainTopY, Math.min(trainBottomY, newY));
            } else {
                // If somehow off the train while moving, drag them back relative to train speed
                p.x -= (lobby.train.speed * 10) * data.dt; 
                p.y = newY;
            }
        } else {
            // Free movement when stopped
            p.x = newX; 
            p.y = newY;
        }
    ...
    socket.on('disconnect', () => {
        ...
        if (lobby && lobby.players[socket.id]) {
            ...
            io.to(socket.lobbyId).emit('update_players', lobby.players);
